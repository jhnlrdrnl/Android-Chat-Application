...
public class MessageAdapter extends RecyclerView.Adapter {
    ...
    public MessageAdapter (Activity activity, LayoutInflater inflater) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        try {
            if (message.getBoolean("isSent")) {
                ...
                notification(message.getString("name"));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void notification(String name) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel("Sockets", "Message from: " + name, NotificationManager.IMPORTANCE_DEFAULT);
            NotificationManager manager = messageActivity.getSystemService(NotificationManager.class);
            manager.createNotificationChannel(channel);
        }

        NotificationCompat.Builder builder = new NotificationCompat.Builder(messageActivity.getApplicationContext(), "Sockets");
        builder.setContentTitle("Socket");
        builder.setContentText("Message from: " + name);
        builder.setSmallIcon(android.R.drawable.stat_notify_sync);
        builder.setAutoCancel(true);

        NotificationManagerCompat managerCompat = NotificationManagerCompat.from(messageActivity.getApplicationContext());
